...
class BoardCanvas(tk.Canvas):
    ...
    def _draw_highlights(self) -> None:
        if self.controller.last_move:
            for sq in (
                self.controller.last_move.from_square,
                self.controller.last_move.to_square,
            ):
        ...
        if hasattr(self.controller, "get_hint_move"):
            hint = self.controller.get_hint_move()
            level = self.controller.get_hint_level()
            if hint is not None and level > 0:
                from config import HINT_COLOR
                if level >= 2:
                    self._highlight_square(hint.from_square, HINT_COLOR, style="hint")
                self._highlight_square(hint.to_square, HINT_COLOR, style="hint")
    ...
    def _highlight_square(
        self, square: chess.Square, color: str, style: str = "soft"
    ) -> None:
    ...
    def animate_bot_move(
        self, move: chess.Move, on_finished: Callable[[], None]
    ) -> None:
        ...
        self._anim_step = 0
        self._anim_piece_key = (
            ("w" if piece.color == chess.WHITE else "b") + piece.symbol().upper()
        )
        ...
        if img:
            self._anim_item = self.create_image(
                self._anim_start_xy[0],
                self._anim_start_xy[1],
                image=img,
                tags=("flying",),
            )
